...
def pointing_frame(src,t):
    # 4.03s impact cut: fast overshoot then a restrained Ken-Burns drift.
    local=max(0,t-4.03)
    q=b.ease(min(1,local/.38))
    if q < .72:
        z=b.lerp(1.00,1.115,b.ease(q/.72))
    else:
        z=b.lerp(1.115,1.065,b.ease((q-.72)/.28))
    if local>.38:
        z=1.065 + .018*b.ease(min(1,(local-.38)/1.5)) + .006*math.sin(local*3.2)

    base=src.resize((b.W,b.H),Image.Resampling.LANCZOS)
    nw,nh=int(b.W*z),int(b.H*z)
    zoom=base.resize((nw,nh),Image.Resampling.LANCZOS)

    # Slight drift toward the pointing hand while retaining open sky on the right.
    drift=b.ease(min(1,local/1.65))
    cx=nw/2 - 18*drift
    cy=nh/2 - 8*drift
    left=int(max(0,min(nw-b.W,cx-b.W/2)))
    top=int(max(0,min(nh-b.H,cy-b.H/2)))
    frame=zoom.crop((left,top,left+b.W,top+b.H)).convert('RGBA')

    d=ImageDraw.Draw(frame,'RGBA')
    # Finger location after the scene's centered zoom. Approximate, but attached to the real pose.
    fx=(405/960)*nw-left
    fy=(169/540)*nh-top
    pulse=.5+.5*math.sin(local*10)
    for rr,a,w in [(18,230,5),(36,125,6),(62,55,8)]:
        r=rr*(.88+.18*pulse)
        d.ellipse((fx-r,fy-r,fx+r,fy+r),outline=(255,226,54,a),width=w)
    for i in range(8):
        ang=i*math.tau/8+local*2.2
        r=74+9*math.sin(local*5+i)
        b.star(d,fx+math.cos(ang)*r,fy+math.sin(ang)*r,8,(255,166,45,220))

    # Impact streaks only during the cut, then disappear.
    if local<.42:
        a=int(180*(1-local/.42))
        for j in range(6):
            yy=105+j*90
            d.line((0,yy,165,yy-35),fill=(255,255,255,max(0,a-j*12)),width=7)

    # Micro recoil on the whole frame during the overshoot.
    if local<.34:
        shake=5*math.sin(local*55)*(1-local/.34)
        tmp=Image.new('RGBA',(b.W,b.H),(30,130,225,255))
        tmp.alpha_composite(frame,(int(shake),int(-shake*.45)))
        frame=tmp
    return frame
...
def main():
    ...
    pointing=Image.open(POINTING).convert('RGB'); pointing.load()
    ...
    for fi in range(b.N):
        ...
        if t<4.03:
            ...
            bg=pointing_frame(pointing,t)
        else:
            bg=b.prism(t)

        # Kinetic title remains stable across the face -> pointing-scene cut.
        if 1.6<t<6.02:
        ...
        if 6.02<t<7.35:
            bg=b.cloud_wipe(bg,t)

        # 3-frame warm flash seals the hard cut at 4.03 s.
        if 4.03<=t<4.16:
    ...
    print('P4_B4_V5_PASS',out,out.stat().st_size,dur)
